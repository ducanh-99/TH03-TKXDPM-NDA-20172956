		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (controller instanceof UserMediaPageController) {
					...
					float weight = 0;
					if (singlePane.getData() instanceof PhysicalMedia) {
						PhysicalMedia physicalMedia = (PhysicalMedia) singlePane.getData();
						weight = physicalMedia.getWeight();
					}
					((UserMediaPageController) controller).addToCart(singlePane.getData().getId(),
							singlePane.getData().getTitle(), singlePane.getData().getCost(), (int) spin.getValue(),
							weight);
				}
			}
		});
