...
public class MediaApi {
	...
	public DigitalVideoDisc updateDigitalVideoDisc(DigitalVideoDisc dvd) {
		WebTarget webTarget = client.target(PATH).path("dvds").path(dvd.getId());

		Invocation.Builder invocationBuilder = webTarget.request(MediaType.APPLICATION_JSON);
		Response response = invocationBuilder.post(Entity.entity(dvd, MediaType.APPLICATION_JSON));

		DigitalVideoDisc res = response.readEntity(DigitalVideoDisc.class);
		return res;
	}
}
